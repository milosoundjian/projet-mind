...
from pmind.replay import test_rb_noise_level
...
def main():
    ...
    # ---- load replay buffers ----
    rb = torch.load(
        f"/Vrac/21500050/projet-mind/experiments/models/{ENV_NAME}/rb-{EXPLOIT_REWARD}-noise-{NOISE}.pt",
        weights_only=False,
    )
    ...
    # ---- run ----
    print(f"seed: {SEED}")
    result = test_rb_noise_level(rb, NOISE, BUFFER_SIZE, TD3, cfg_offline, SEED)

    # ---- save ----
    output_path = f"/users/Etu0/21500050/results/result-{ENV_NAME}-{EXPLOIT_REWARD}-{NOISE}-{SEED}.pt"
